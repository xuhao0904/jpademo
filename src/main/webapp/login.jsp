...
<script type="text/javascript">
    function login(){
    	$.getJSON("${pageContext.request.contextPath}/employee/login",{"username":$("#username").val(),"password":$("#pass").val()},function (data) {
    		if(data.flag){
				if(data.messages!=null &&data.messages!=undefined){
					location.href=data.messages
				}else {	location.href="${pageContext.request.contextPath}/index.jsp"}
				}else{
				alert(data.messages);
			}
			...
			<TBODY>
				...
				<TR>
					...
					<TD rowSpan=3><IMG height=180 alt="" src="images/crm_16.gif"
						width=24></TD>
					<TD><input type="button" onclick="login()"></TD>
