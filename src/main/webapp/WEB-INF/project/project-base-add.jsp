...
<tr >
	...
	<td align='left' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22"><input type="text"  id="com" name="comper"/></td>
...
<script type="text/javascript">
	...
	var custs=null;
	$(function () {
		$.getJSON("${pageContext.request.contextPath}/project/addProject",{"name":"项目经理"},function (data) {
			custs=data.customer;
			$.each(data.customer, function (index,element) {
				var select="<option value="+element.id+">"+element.comname+"</option>";
				$("#cust").append(select);
			});
			$.each(data.employee,function (index,element) {
				var select="<option  value="+element.eid+">"+element.ename+"</option >";
				$("#emp").append(select);
			})
	...
	$("#cust").change(function (){
		var val = $(this).val();
		$.each(custs,function (index,element) {
			if(element.id==val){
				$("#com").val(element.companyperson)
			}
		})
